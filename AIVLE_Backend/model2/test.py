...
def main():
    ...
    ## testing & inference
    ckpt_path = os.path.join('checkpoint_2_300.tar') # chceck your path
    ...
    model.load_state_dict(ckpt['model_state_dict']); model.to(main_device); print(f"평가 수행 : {time.ctime()}")
    test_loss, test_acc, (AUROC, AUPRC, TH_ACC, RECALL, PRECISION, F1, BRIER, CM), (predicted_probas, labels)= inference_evaluate(model, 
                                                                                                                                  main_device, 
                                                                                                                                  criterion, 
    ...
    print(f"평가 종료: {time.ctime()}")
    prediction_values = pd.DataFrame({'데이터ID':test_file_ids,
                                      '모델 예측값':predicted_probas, 
                                      '모델 예측 Label':np.where(predicted_probas >= inference_label_frequency, 1, 0),
                                      'GT 값':labels})
